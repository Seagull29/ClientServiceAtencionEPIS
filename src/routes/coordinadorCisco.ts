...
router.get('/', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
    const solicitudes = await Solicitud.search('cisco', 'categoria');
    const prioridades = await Prioridad.list();
    const tipos = await Tipo.list();
    res.render('coorCisco/cisco', {
        user: req.user,
        solicitudes,
        prioridades,
        tipos,
...
router.get('/solicitud/detalles/:id', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
    ...
    res.render('coorCisco/solicitud', {
        user,
...
router.get('/descarga/:id', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
    const { id } = req.params;
...
router.post('/solicitud/nuevo-mensaje', isLoggedIn, authRole(ROLES.CCISCO), upload.array('archivo-mensaje', 5), async (req : Request, res : Response) => {
...
router.get('/solicitud/cerrar/:id', isLoggedIn, authRole(ROLES.CCISCO), async (req: Request, res: Response) => {
    const { id } = req.params;
...
router.get('/prioridad-filtro/:prioridad', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
    const { prioridad } = req.params;
    const solicitudes = await Solicitud.search(prioridad, 'prioridad');
    const tipos = solicitudes.filter(solicitud =>  solicitud.Categoria === 'Academia CISCO');
    res.json(tipos);
});

router.get('/tipo-filtro/:tipo', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
...
router.get('/en-proceso', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
...
router.get('/atendido', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
...
router.get('/enviado', isLoggedIn, authRole(ROLES.CCISCO), async (req : Request, res : Response) => {
